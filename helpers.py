...
def get_weighted_boost_avg(pokemon_state, weights, total_weight_sum):
    ...
    if not pokemon_state: return 0.0
...
def get_type_multiplier(move_type, target_types):
    ...
    if move_type == 'unknown' or not target_types: return 1.0
    chart = GEN1_TYPE_CHART.get(move_type.lower(), {})
    multiplier = 1.0
    for target_type in target_types:
        if target_type == 'notype': continue
...
def pokemon_win_rate(pokemon_dictionary, battle_data):
    ...
    for battle in battle_data:
        if 'player_won' not in battle: continue
    ...
    for name, stats in pokemon_stats.items():
        if stats['plays'] > 0:
            pokemon_stats[name]['win_rate_float'] = (stats['wins'] / stats['plays']) * 100
        else:
    ...
    for battle in battle_data:
        ...
        for turn in timeline:
            for p in ['p1', 'p2']:
                ...
                if state and move:
                    pokemon_name, move_name, move_type = state.get('name'), move.get('name'), move.get('type')
