...
def compute_choice_features(battle):
    ...
    if len(timeline) <= 1:
        return {'p1_switch_count': 0, 'p2_switch_count': 0, 'switch_advantage': 0,
                'p1_boost_move_count': 0, 'p2_boost_move_count': 0, 'boost_move_advantage': 0}

    p1_switches, p2_switches, p1_boost_moves, p2_boost_moves = 0, 0, 0, 0
    for i in range(1, len(timeline)):
        prev_turn, curr_turn = timeline[i-1], timeline[i]
        prev_state_p1, curr_state_p1 = prev_turn.get('p1_pokemon_state', {}), curr_turn.get('p1_pokemon_state', {})
        prev_state_p2, curr_state_p2 = prev_turn.get('p2_pokemon_state', {}), curr_turn.get('p2_pokemon_state', {})
        if not (prev_state_p1 and curr_state_p1 and prev_state_p2 and curr_state_p2):
            continue
        if prev_state_p1.get('name') != curr_state_p1.get('name') and prev_state_p1.get('status') != 'fnt':
            p1_switches += 1
        if prev_state_p2.get('name') != curr_state_p2.get('name') and prev_state_p2.get('status') != 'fnt':
            p2_switches += 1
        if sum(curr_state_p1.get('boosts', {}).values()) > sum(prev_state_p1.get('boosts', {}).values()):
            p1_boost_moves += 1
        if sum(curr_state_p2.get('boosts', {}).values()) > sum(prev_state_p2.get('boosts', {}).values()):
            p2_boost_moves += 1
    return {
...
def compute_tactical_features(battle):
    ...
    turns = len(timeline)
    if turns <= 1:
        return {"hp_delta_balance": 0.0, "status_balance": 0.0,
                "momentum_stability": 0.0, "avg_damage_turn": 0.0}
    ...
    for i in range(1, turns):
        ...
        prev_state_p1, curr_state_p1 = prev.get('p1_pokemon_state', {}), curr.get('p1_pokemon_state', {})
        prev_state_p2, curr_state_p2 = prev.get('p2_pokemon_state', {}), curr.get('p2_pokemon_state', {})
        if not (prev_state_p1 and curr_state_p1 and prev_state_p2 and curr_state_p2):
            continue
    ...
    if not damage_ratios or np.mean(np.abs(damage_ratios)) < 1e-5:
        ...
        momentum_stability = 1 - (np.var(damage_ratios) / 
                                 (np.mean(np.abs(damage_ratios)) + 1e-5))
...
def compute_endgame_features(battle_data, pokemon_type_map, observed_moves_map, move_to_type_map):
    """Calculates final snapshot features: remaining counts, total HP, and type advantage."""
    features = {}
    timeline = battle_data.get("battle_timeline", [])
    if not timeline:
        return {'remaining_pokemon_advantage': 0, 'total_hp_advantage': 0, 't30_type_advantage': 0}
    ...
    for turn in timeline:
        ...
        if p1_state and p1_state.get('name'): last_known_state[p1_state['name']] = p1_state
    ...
    for name, details in p1_team_map.items():
        ...
        if not is_fainted:
            ...
            p1_remaining_squad.append({
                'name': name, 'types': details.get('types', []),
                'hp_pct': hp_pct,
    ...
    for name in p2_revealed_names:
        ...
        if state and state.get('status') != 'fnt' and state.get('hp_pct', 0.0) > 0.0:
            ...
            p2_remaining_squad.append({
                'name': name, 'types': pokemon_type_map.get(name, []),
    ...
    features['remaining_pokemon_advantage'] = len(p1_remaining_squad) - len(p2_remaining_squad)
    ...
    for p1_mon in p1_remaining_squad:
        for p2_mon in p2_remaining_squad:
            ...
            for move_name in p1_mon['moves']:
                ...
                best_p1_offense = max(best_p1_offense, get_type_multiplier(move_type, p2_mon['types']))
            for move_name in p2_mon['moves']:
                move_type = move_to_type_map.get(move_name, 'unknown')
                best_p2_offense = max(best_p2_offense, get_type_multiplier(move_type, p1_mon['types']))
...
def process_battle_base(battle_data, pokedex, stats_map):
    ...
    for turn in timeline:
        ...
        if p2_state:
            ...
            if pokemon_name: p2_revealed_names.add(pokemon_name)
    ...
    p1_win_rates = [stats_map.get(p['name'], {}).get('win_rate_float', 50.0) for p in p1_team]
    ...
    stats_to_avg = ['base_hp', 'base_atk', 'base_def', 'base_spa', 'base_spd', 'base_spe']
    for stat in stats_to_avg:
        p1_avg_stat = np.mean([p.get(stat, DEFAULT_STAT_VALUE) for p in p1_team])
        p2_avg_stat = np.mean([p.get(stat, DEFAULT_STAT_VALUE) for p in p2_team_stats_list])
    ...
    features['t30_p1_hp_pct'] = p1_last_state.get('hp_pct', 0.0) if p1_last_state else 0.0
    features['t30_p2_hp_pct'] = p2_last_state.get('hp_pct', 0.0) if p2_last_state else 0.0
    
    # --- 7. T30 Status (Restored text for get_dummies) ---
    features['t30_p1_status'] = p1_last_state.get('status', 'ok') if p1_last_state else 'ok'
    features['t30_p2_status'] = p2_last_state.get('status', 'ok') if p2_last_state else 'ok'
    
    p1_weighted_boost_avg = get_weighted_boost_avg(p1_last_state, BOOST_WEIGHTS, TOTAL_BOOST_WEIGHT_SUM)
    p2_weighted_boost_avg = get_weighted_boost_avg(p2_last_state, BOOST_WEIGHTS, TOTAL_BOOST_WEIGHT_SUM)
    features['t30_weighted_boost_advantage'] = p1_weighted_boost_avg - p2_weighted_boost_avg
