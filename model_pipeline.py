...
def run_model_1_logistic_net(X_processed, y):
    ...
    print(classification_report(y_test, y_pred_test, target_names=['loss (0)', 'win (1)']))
...
def run_model_2_calibrated_stacking(X_processed, y):
    ...
    pipeline_lr = Pipeline(steps=[
        ('scaler', StandardScaler()),
        ('model_lr', LogisticRegression(random_state=42, solver='saga', max_iter=3000))
    ])
    param_grid_lr = {
    ...
    # --- 4. Tune the experts ---
    best_lr = tune_model("LR (Stack)", pipeline_lr, param_grid_lr, X_train_val, y_train_val, cv_strategy)
    best_svm = tune_model("SVM (Stack)", pipeline_svm, param_grid_svm, X_train_val, y_train_val, cv_strategy)
    best_xgb = tune_model("XGB (Stack)", pipeline_xgb, param_grid_xgb, X_train_val, y_train_val, cv_strategy)

    # --- 5. CALIBRATE THE EXPERTS ---
    print("\n--- Calibrating uncalibrated models (SVM, XGBoost) ---")
    calibrated_svm = CalibratedClassifierCV(best_svm, method='isotonic', cv=cv_strategy, n_jobs=-1)
    calibrated_xgb = CalibratedClassifierCV(best_xgb, method='isotonic', cv=cv_strategy, n_jobs=-1)
    ...
    meta_preds_lr = cross_val_predict(
        best_lr, X_train_val, y_train_val, cv=cv_strategy, 
        method='predict_proba', n_jobs=-1)[:, 1]
    meta_preds_svm_cal = cross_val_predict(
        calibrated_svm, X_train_val, y_train_val, cv=cv_strategy, 
        method='predict_proba', n_jobs=-1)[:, 1]
    meta_preds_xgb_cal = cross_val_predict(
        calibrated_xgb, X_train_val, y_train_val, cv=cv_strategy, 
        method='predict_proba', n_jobs=-1)[:, 1]
    ...
    print("\nClassification report on test set")
    print(classification_report(y_test, final_predictions, target_names=['loss (0)', 'win (1)']))
...
def run_model_3_calibrated_voting(X_processed, y):
    ...
    param_grid_lr = {
        'model_lr__penalty': ['elasticnet'], 'model_lr__C': [0.1, 1.0, 10.0], 'model_lr__l1_ratio': [0.1, 0.5, 0.9]
    }
    pipeline_svm = Pipeline(steps=[
        ('scaler', StandardScaler()),
        ('model_svm', SVC(random_state=42, probability=True))
    ...
    param_grid_svm = {
        'model_svm__C': [0.1, 1.0, 10.0], 'model_svm__kernel': ['rbf', 'linear']
    }
    pipeline_lgbm = Pipeline(steps=[
        ('model_lgbm', LGBMClassifier(random_state=42, n_jobs=-1, verbose=-1))
    ])
    param_grid_lgbm = {
        'model_lgbm__n_estimators': [100, 200], 'model_lgbm__max_depth': [4, 6], 'model_lgbm__learning_rate': [0.1]
    }

    # Tune models
    best_lr = tune_model("LR (Voting)", pipeline_lr, param_grid_lr, X_train_val, y_train_val, cv_strategy)
    best_svm = tune_model("SVM (Voting)", pipeline_svm, param_grid_svm, X_train_val, y_train_val, cv_strategy)
    best_lgbm = tune_model("LGBM (Voting)", pipeline_lgbm, param_grid_lgbm, X_train_val, y_train_val, cv_strategy)

    # Calibrate
    print("\n--- Calibrating uncalibrated models (SVM, LGBM) ---")
    calibrated_svm = CalibratedClassifierCV(best_svm, method='isotonic', cv=cv_strategy, n_jobs=-1)
    calibrated_lgbm = CalibratedClassifierCV(best_lgbm, method='isotonic', cv=cv_strategy, n_jobs=-1)
    print("Fitting SVM calibrator...")
    calibrated_svm.fit(X_train_val, y_train_val)
